            ...
            <ul class="flex space-x-6 lg:space-x-8">
              <li v-for="item in menuitems" :key="item.title" class="relative">
                <NuxtLink :to="item.path" @mouseenter="item.hasDropdown ? workDropdownOpen = true : null"
                  ...
                  class="text-sm lg:text-base transition-colors duration-200 flex items-center whitespace-nowrap font-medium text-white hover:text-[#adb3a1]">
                  {{ item.title }}
                ...
                <div v-if="item.hasDropdown && workDropdownOpen" @mouseenter="workDropdownOpen = true"
                  ...
                  <div class="space-y-4">
                    <NuxtLink v-for="subitem in item.subitems" :key="subitem.title" :to="subitem.path"
                      class="group hover:bg-white/10 transition-all duration-300 p-4 rounded-lg flex gap-4">
                      <!-- Image on the left -->
                      <div class="w-32 h-24 bg-gray-700 rounded-lg overflow-hidden flex-shrink-0">
                        <img :src="subitem.image" :alt="subitem.title"
                          class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      </div>
                      <!-- Text on the right -->
                      <div class="flex-1">
                        <h3 class="text-lg font-bold text-white mb-2">{{ subitem.title }}</h3>
