<template>
  <main class="relative w-screen h-screen overflow-hidden bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800">
    ...
    <div class="absolute right-0 md:right-0 top-0 w-1/2 md:w-1/2 h-full overflow-hidden">
      <!-- Gradient overlay now properly behind image on mobile -->
      <div
        class="absolute inset-0 bg-gradient-to-l md:bg-gradient-to-l from-transparent via-slate-900/40 md:via-transparent to-slate-900/80 md:to-slate-900/20 z-10">
      </div>
      <img src="/img/wilsonsamson-hero-image.png" alt="Wilson Samson"
        class="w-full h-full object-cover object-center opacity-90 relative z-20" />
    </div>
...
.logo-enhanced {
  ...
  filter:
    contrast(1.3) brightness(1.2) saturate(1.1) drop-shadow(0 0 8px rgba(255, 255, 255, 0.1));
...
/* Fade only animation for descriptions */
...
@media (max-width: 768px) {
  ...
  /* Center content on mobile and move down */
  ...
  .relative.z-20.w-full.h-full.flex {
    padding-top: 15vh;
...
@media (max-width: 640px) {
  ...
  /* Push content down further on very small screens */
  .relative.z-20.w-full.h-full.flex {
    padding-top: 10vh;
  }
  ...
  /* Move signature to bottom left on mobile */
  .signature-text {
    position: absolute;
    bottom: 8px;
    left: 8px;
