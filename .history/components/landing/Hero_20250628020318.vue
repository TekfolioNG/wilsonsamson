<template>
  <main class="relative w-screen h-screen overflow-hidden bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800">
    ...
    <div class="absolute right-0 md:right-0 top-0 w-1/2 md:w-1/2 h-full overflow-hidden">
      ...
      <!-- Enhanced gradient overlay - Fixed mobile width -->
      <div
        class="absolute inset-0 bg-gradient-to-l md:bg-gradient-to-l from-transparent via-slate-900/40 md:via-transparent to-slate-900/60 md:to-slate-900/20">
      </div>
    </div>
...
@media (max-width: 768px) {
  ...
  .mobile-content-center {
    ...
    /* Changed from -3vh to +8vh to push content down */
    margin-left: auto;
    margin-right: auto;
    max-width: 45%;
    /* Constrain width to prevent overlap with image */
  }
  ...
  /* Ensure description text is centered and constrained */
  .hero-description {
    text-align: center;
  ...
  .logo-container {
    opacity: 0.15;
    left: 20px;
    /* Moved closer to edge to give more space */
    right: auto;
  }
...
@media (max-width: 640px) {
  ...
  /* More aggressive content positioning for smaller screens */
  .mobile-content-center {
    transform: translateY(10vh);
    /* Push down even more on smaller screens */
    max-width: 42%;
    /* Further constrain width */
  ...
  /* Keep logo visible but smaller on mobile */
  .logo-container {
    display: block;
    opacity: 0.1;
    left: 10px;
  ...
  .logo-enhanced {
    width: 5rem;
    /* Even smaller for tight screens */
    height: 5rem;
  }
}
