...
<template>
  <div class="fixed w-full z-50 top-0 backdrop-blur-md border-b border-gray-200/30 dark:border-gray-800/30">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="hidden md:flex items-center">
          <div v-for="item in menuitems" :key="item.title" class="flex items-center">
            <NuxtLink :to="item.path"
              class="px-4 py-2 text-sm lg:text-base font-medium text-gray-900 dark:text-white hover:text-primary dark:hover:text-primary transition-colors duration-200 flex items-center">
              {{ item.title }}
        ...
        <div class="hidden md:flex items-center ml-4">
          <ClientOnly v-if="!colorMode?.forced">
            <UButton :icon="isDark ? 'i-lucide-moon' : 'i-lucide-sun'" color="neutral" variant="ghost"
              @click="isDark = !isDark" class="text-gray-900 dark:text-white" />
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open"
          class="md:hidden bg-white/90 dark:bg-gray-900/90 backdrop-blur-md rounded-lg shadow-lg mt-2 py-2 border border-gray-100 dark:border-gray-800">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path"
              class="block px-4 py-3 text-sm font-medium text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-800 flex justify-between"
              @click="open = false">
              {{ item.title }}
