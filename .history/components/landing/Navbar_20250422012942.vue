<template>
  <div class="fixed w-full z-50 top-0 left-0 right-0">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Change this to use flex with three equal sections -->
      <header class="flex items-center py-3">
        ...
        <!-- Desktop Navigation - centered in middle 1/3 -->
        <nav class="hidden md:flex items-center justify-center w-1/3 mx-auto">
          <div v-for="item in menuitems" :key="item.title" class="flex items-center px-2">
            <NuxtLink :to="item.path"
              class="px-2 py-2 text-sm lg:text-base font-medium text-white hover:text-primary transition-colors duration-200 flex items-center"
