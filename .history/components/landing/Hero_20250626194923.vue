  ...
  <main class="relative w-screen h-screen overflow-hidden">
    ...
    <div class="relative z-20 w-full h-full flex flex-col px-6 md:pl-16 lg:pl-24 xl:pl-32">
      ...
      <div
        ...
        :class="{ 'items-end text-right': currentSlide === 0 }">
        ...
        <transition name="fade" mode="out-in">
          <p v-if="slides[currentSlide].description" :key="'desc-' + currentSlide"
            ...
            :style="{ backgroundColor: 'rgba(153, 204, 51, 0.7)', backdropFilter: 'blur(2px)', textShadow: '0 2px 4px rgba(0,0,0,0.8), 0 0 8px rgba(0,0,0,0.5)' }">
            {{ slides[currentSlide].description }}
          </p>
        </transition>
        <transition name="fade" mode="out-in">
          <div v-if="slides[currentSlide].buttonText" :key="'button-' + currentSlide" class="mt-8 w-full flex"
            :class="{ 'justify-end': currentSlide === 0, 'justify-center md:block': currentSlide !== 0 }">
...
@media (max-width: 767px) {
  ...
  .hero-description {
    ...
    line-height: 1.5;
    padding: 0.75rem;
    background: rgba(153, 204, 51, 0.7) !important;
