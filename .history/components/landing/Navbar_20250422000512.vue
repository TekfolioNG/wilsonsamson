...
<template>
  <div
    class="fixed w-full z-50 top-0 backdrop-blur-md bg-white/60 dark:bg-gray-900/60 border-b border-gray-200 dark:border-gray-800">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex justify-between items-center py-4">
        <!-- Logo -->
        <a href="/" class="flex-shrink-0">
          <img src="~/assets/img/PPIL Logo.svg" alt="PPIL Logo" class="h-14 md:h-16 dark:invert" />
        </a>
        ...
        <nav class="hidden md:flex items-center space-x-2 lg:space-x-4">
          <div v-for="(item, index) in menuitems" :key="item.title" class="flex items-center">
            <NuxtLink :to="item.path"
              class="px-3 py-2 text-sm lg:text-base font-medium text-gray-900 dark:text-gray-100 hover:text-primary transition-colors duration-200 flex items-center">
              {{ item.title }}
            ...
            <span v-if="index !== menuitems.length - 1" class="text-gray-300 dark:text-gray-700 text-xs mx-1">|</span>
          </div>
        </nav>
        ...
        <div class="hidden md:flex items-center ml-4">
          <ClientOnly v-if="!colorMode?.forced">
            <UButton :icon="isDark ? 'i-lucide-moon' : 'i-lucide-sun'" color="neutral" variant="ghost"
              @click="isDark = !isDark" class="text-gray-900 dark:text-gray-100" />
        ...
        <div class="md:hidden flex items-center">
          ...
          <!-- Hamburger Menu -->
          <button @click="open = !open" class="text-gray-900 dark:text-gray-100 focus:outline-none">
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open"
          class="md:hidden bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-lg shadow-lg mt-2 py-2 border border-gray-100 dark:border-gray-800">
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path"
              class="block px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800 flex justify-between"
              @click="open = false">
              {{ item.title }}
