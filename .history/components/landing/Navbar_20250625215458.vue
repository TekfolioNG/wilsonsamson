    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center justify-between py-8">
        ...
        <div class="hidden md:flex items-center">
            ...
            <ul class="flex space-x-6 lg:space-x-8">
              <li v-for="item in menuitems" :key="item.title" class="relative">
                ...
                <div v-if="item.hasDropdown && workDropdownOpen" @mouseenter="workDropdownOpen = true"
                  ...
                  class="absolute left-1/2 transform -translate-x-1/2 top-full mt-0 w-screen max-w-6xl bg-black/90 backdrop-blur-sm rounded-b-lg shadow-xl z-50 p-8">
          ...
          <NuxtLink to="/partner"
            class="ml-12 px-6 py-3 rounded-md text-sm lg:text-base font-medium text-white bg-[#a86414] hover:bg-[#74b41a] transition-colors duration-200 whitespace-nowrap">
            Partner with Us
          </NuxtLink>
        </div>
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg bg-white/95">
          <div v-for="item in menuitems" :key="item.title" class="block">
            ...
            <div v-if="item.hasDropdown && open" class="pl-6 bg-black/5 rounded-lg mx-4 my-2">
              <NuxtLink v-for="subitem in item.subitems" :key="subitem.title" :to="subitem.path"
                class="block px-4 py-3 text-sm text-gray-700 hover:text-[#a86414] border-b border-gray-100 last:border-0"
                @click="open = false">
          ...
          <NuxtLink to="/partner"
            class="block px-4 py-3 mt-2 mx-4 rounded-md text-sm font-medium text-white bg-[#a86414] hover:bg-[#74b41a]"
            @click="open = false">
            Partner with Us
          </NuxtLink>
        </div>
