<template>
  <main class="relative w-screen h-screen overflow-hidden">
    ...
    <!-- Content overlay - full width with adjusted positioning -->
    <div class="relative z-20 w-full h-full flex flex-col px-6 md:px-16 lg:px-24 xl:px-32">
      <div class="flex flex-col mt-auto mb-32 md:mb-40 lg:mb-48 max-w-3xl">
        ...
        <transition name="fade" mode="out-in">
          <p key="desc" class="mt-6 text-lg md:text-xl text-gray-100 max-w-2xl font-light leading-relaxed">
            {{ slides[currentSlide].description }}
          </p>
        </transition>
        <div class="mt-8 flex flex-wrap gap-4">
          <NuxtLink :to="slides[currentSlide].primaryLink"
            class="px-8 py-3 bg-yellow text-black hover:bg-yellow-500 font-medium transition-all duration-300 text-lg transform hover:scale-105">
            {{ slides[currentSlide].primaryText }}
          </NuxtLink>
          <NuxtLink :to="slides[currentSlide].secondaryLink"
            class="px-8 py-3 border-2 border-white text-white hover:bg-white hover:text-black transition-all duration-300 font-medium text-lg transform hover:scale-105">
