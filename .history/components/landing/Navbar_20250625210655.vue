    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex items-center justify-between py-1">
        ...
        <NuxtLink to="/" class="inline-block">
          <img src="~/assets/img/veaverde-logo.png" alt="VeraVerde Logo" class="h-24 md:h-18 drop-shadow-lg" />
        </NuxtLink>
          ...
          <ul class="flex space-x-6 lg:space-x-8">
            <li v-for="item in menuitems" :key="item.title">
              <NuxtLink :to="item.path"
                class="text-sm lg:text-base transition-colors duration-200 flex items-center whitespace-nowrap font-medium text-white hover:text-[#E6A619]">
                {{ item.title }}
