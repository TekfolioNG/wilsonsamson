          ...
          <ul class="flex space-x-6 lg:space-x-8">
            <li v-for="item in menuitems" :key="item.title">
              <NuxtLink :to="item.path"
                class="text-sm lg:text-base transition-colors duration-200 flex items-center whitespace-nowrap"
                :class="{
                  'text-black font-medium': (!isDark && scrolled),
      ...
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg"
          ...
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path" class="block px-4 py-3 text-sm flex justify-between"
              :class="{ 
                'text-black font-medium hover:text-primary': !isDark && scrolled, 
                'text-white hover:text-primary': !isDark && !scrolled, 
                'text-white hover:text-yellow-400': isDark 
              }"
              @click="open = false">
              {{ item.title }}
