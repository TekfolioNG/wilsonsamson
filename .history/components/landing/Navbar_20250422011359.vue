  ...
  <div class="fixed w-full z-50 top-0"
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <header class="flex justify-between items-center py-4">
        ...
        <a href="/" class="flex-shrink-0 bg-white/30 dark:bg-black/30 p-2 rounded-lg backdrop-blur-md shadow-sm">
        ...
        <div class="hidden md:flex items-center ml-4">
          <ClientOnly v-if="!colorMode?.forced">
            <UButton :icon="isDark ? 'i-lucide-moon' : 'i-lucide-sun'" color="neutral" variant="ghost"
              :class="{ 'text-gray-900': !isDark, 'text-white': isDark }" @click="isDark = !isDark" />
            <template #fallback>
              <div class="size-8" />
            </template>
          </ClientOnly>
