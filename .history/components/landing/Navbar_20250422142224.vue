...
<template>
  <div class="fixed w-full z-50 top-0 left-0 right-0 transition-all duration-300"
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Mobile Menu with updated text colors and styling -->
      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
        ...
        <div v-show="open" class="md:hidden mt-2 py-2 rounded-lg shadow-lg"
          ...
          <div v-for="item in menuitems" :key="item.title" class="block">
            <NuxtLink :to="item.path"
              class="block px-4 py-3 text-sm font-medium transition-colors duration-200 flex justify-between items-center whitespace-nowrap"
              :class="{
                'text-black hover:text-primary': !isDark && scrolled,
                'text-white hover:text-primary': (!isDark && !scrolled) || (isDark && !scrolled) || (isDark && scrolled)
              }" @click="open = false">
              {{ item.title }}
              <span v-if="item.hasDropdown" class="ml-1">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" :class="{
                  'text-black': !isDark && scrolled,
                  'text-white': (!isDark && !scrolled) || isDark
                }">
                  <path fill-rule="evenodd"
...
/* Ensure mobile text is visible in all states */
