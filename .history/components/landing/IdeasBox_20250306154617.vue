    ...
    <div class="min-h-screen bg-white py-16 px-4">
        <div class="max-w-5xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- Form section -->
                <div
                    class="bg-emerald-50 rounded-xl shadow-lg overflow-hidden transform transition-all duration-300 hover:shadow-xl h-120">
                    <div class="p-8">
                        ...
                        <form @submit.prevent="submitIdea" class="space-y-5">
                            ...
                            <div>
                                <label for="description" class="block text-sm font-semibold text-gray-700 mb-2">Idea
                                    Description</label>
                                <textarea v-model="ideaDescription" id="description" rows="6"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                ...
                <!-- Animation container -->
                <div class="bg-emerald-50 rounded-xl shadow-lg overflow-hidden h-120 relative">
                    <div class="animation-container relative w-full h-full flex items-center justify-center">
                        <!-- Paper document - positioned at top left initially -->
                        <div ref="paper"
                            class="absolute w-40 h-52 bg-white border-2 border-gray-300 rounded-md shadow-md z-10 transform-gpu"
                            :style="paperStyle">
                            ...
                            <div v-if="paperDescriptionLine3"
                                class="w-3/4 h-1.5 bg-gray-700 mx-auto mt-3 transition-all duration-300"
                                :style="{ width: paperDescLine3Width }"></div>
                            ...
                            <!-- Signature line -->
                            <div class="w-2/3 h-1.5 bg-gray-300 mx-auto mt-6"></div>

                            <!-- Animated pen when typing -->
                            <div v-if="isTyping" ref="pen"
                                class="absolute w-6 h-1 bg-blue-600 rounded-full transform rotate-45" :style="penStyle">
                                <div class="absolute top-0 right-0 w-1 h-4 bg-blue-600 rounded-full"></div>
                            </div>
                        </div>

                        <!-- Ideas Box - positioned lower in the container -->
                        <div class="absolute bottom-32 w-48 h-36">
                            <!-- Box back -->
                            <div
                        ...
                        <div v-if="submissionStatus"
                            class="absolute bottom-16 w-full text-center text-lg font-medium px-4 py-2 rounded-lg transition-all duration-500"
                        ...
                        <div v-if="!isAnimating && !animationComplete"
                            class="text-center text-gray-500 absolute bottom-20">
                            Submit your idea using the button below to see the animation
                        </div>

                        <!-- Bottom submit button (for animation container) -->
                        <div class="absolute bottom-6 w-full flex justify-center">
                            <button @click="triggerSubmit"
                                class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-8 rounded-lg inline-block transition-colors shadow-md hover:shadow-lg transform hover:-translate-y-1 duration-200"
                                :class="{ 'opacity-50 cursor-not-allowed': isAnimating || !formIsValid }"
                                :disabled="isAnimating || !formIsValid">
                                Submit Idea
                            </button>
                        </div>
                    </div>
...
// Validate if form has required fields
const formIsValid = computed(() => {
    return ideaSubject.value.trim() !== '' &&
        ideaDescription.value.trim() !== '' &&
        email.value.trim() !== '' &&
        /\S+@\S+\.\S+/.test(email.value); // Basic email validation
});
...
const paperDescriptionLine3 = ref(false);
...
const paperDescLine3Width = ref('0%');
...
const currentPenPosition = ref({ x: 20, y: 25 });
const targetPenPosition = ref({ x: 20, y: 25 });
const penStyle = computed(() => ({
    left: `${currentPenPosition.value.x}px`,
    top: `${currentPenPosition.value.y}px`,
}));

// Paper positioned initially at top-left corner, higher up
const paperStyle = computed(() => ({
    top: isAnimating.value ? '55%' : '20%',
    left: isAnimating.value ? '50%' : '25%',
...
// Animated writing with pen
const animatePen = (x, y) => {
    isTyping.value = true;
    targetPenPosition.value = { x, y };

    // Start moving the pen
    if (typingTimer.value) clearTimeout(typingTimer.value);

    const movePen = () => {
        // Calculate step towards target
        const dx = (targetPenPosition.value.x - currentPenPosition.value.x) * 0.2;
        const dy = (targetPenPosition.value.y - currentPenPosition.value.y) * 0.2;

        currentPenPosition.value.x += dx;
        currentPenPosition.value.y += dy;

        // Continue animation if not close enough
        if (Math.abs(dx) > 0.1 || Math.abs(dy) > 0.1) {
            requestAnimationFrame(movePen);
        } else {
            // Hide pen after writing
...
const updatePaperContent = (field) => {
    if (field === 'subject' && ideaSubject.value) {
        ...
        animatePen(20 + ideaSubject.value.length * 1.5, 25);
    }
    ...
    if (field === 'description' && ideaDescription.value) {
        ...
        if (words.length > 0) {
            ...
            animatePen(20 + words.length * 2.5, 35);
        }

        if (words.length > 5) {
            paperDescriptionLine2.value = true;
            paperDescLine2Width.value = Math.min(95, (words.length - 5) * 5) + '%';
            animatePen(20 + (words.length - 5) * 2.5, 45);
        }

        if (words.length > 10) {
            paperDescriptionLine3.value = true;
            paperDescLine3Width.value = Math.min(95, (words.length - 10) * 5) + '%';
            animatePen(20 + (words.length - 10) * 2.5, 55);
        }
    }
    ...
    if (field === 'email' && email.value) {
        ...
        animatePen(20 + email.value.length * 2, 65);
    }
};

// Trigger submit from the animation container button
const triggerSubmit = () => {
    if (!isAnimating.value && formIsValid.value) {
        submitIdea();
    }
};
...
const submitIdea = async () => {
    if (isAnimating.value || !formIsValid.value) return;
    ...
    try {
        ...
        // Paper animation sequence - paper flies from top-left to box
        tl.to(paper.value, {
            top: '30%',
            left: '35%',
            rotation: 10,
...
/* Custom height for taller form and animation sections */
.h-120 {
    height: 30rem;
}
</style>